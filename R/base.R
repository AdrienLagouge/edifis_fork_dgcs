...
exo_fillon1 <<- function(x, bareme) {
  y <<- (x > 0) *
    pmax(
      x *
        ((bareme[["tx_exo_fillon_smic"]] /
          (bareme[["plafond_exo_fillon"]] - 1)) *
          (bareme[["plafond_exo_fillon"]] * (bareme[["smic_b"]] / x) - 1)),
      0
    )
  return(y)
}
...
exo_fillon <<- function(x, y, z, bareme) {
  v <<- ifelse(
    ...
    pmax(
      bareme[["tx_exo_fillon_smic"]] *
        x *
        (bareme[["plafond_exo_fillon"]] * (y / z) - 1) /
        (bareme[["plafond_exo_fillon"]] - 1),
      0
    ),
    0
...
csg_deduc <<- function(x, bareme) {
  y <<- (x > 0) *
    bareme[["taux_csg_deduc_t1"]] *
    pmin(x, 4 * bareme[["plafond_ss"]]) +
    (x > 4 * bareme[["plafond_ss"]]) *
      bareme[["taux_csg_deduc_t2"]] *
      (x - 4 * bareme[["plafond_ss"]])
  return(y)
}
...
csg_non_deduc <<- function(x, bareme) {
  y <<- (x > 0) *
    bareme[["taux_csgcrds_nondeduc_t1"]] *
    pmin(x, 4 * bareme[["plafond_ss"]]) +
    (x > 4 * bareme[["plafond_ss"]]) *
      bareme[["taux_csgcrds_nondeduc_t2"]] *
      (x - 4 * bareme[["plafond_ss"]])
  return(y)
}

choix_input <<- function(
  n0,
  n1,
  n2,
  n3,
  ...
  mf_RSA <<- ifelse(
    (nb_adultes == 1) && (maj_isole_rsa == 0),
    bareme_var[["mont_forfaitaire_rsa"]] *
      (1 +
        (nb_enfants >= 1) * bareme_var[["tx_majo_rsa_pac1"]] +
        (nb_enfants >= 2) * bareme_var[["tx_majo_rsa_enf12"]] +
    ...
    ifelse(
      ...
      bareme_var[["mont_forfaitaire_rsa"]] *
        (1 +
          bareme_var[["tx_majo_rsa_pac1"]] +
          pmin(nb_enfants, 2) * bareme_var[["tx_majo_rsa_enf12"]] +
          pmax(nb_enfants - 2, 0) * bareme_var[["tx_majo_rsa_enf3"]]),
      0
    ) +
    ifelse(
      (nb_adultes == 1) && (maj_isole_rsa == 1),
      bareme_var[["mont_forfaitaire_rsa_maj"]] *
        (1 +
          pmin(nb_enfants, 2) * bareme_var[["tx_majo_rsa_majo_enf12"]] +
          pmax(nb_enfants - 2, 0) * bareme_var[["tx_majo_rsa_majo_enf3"]]),
      0
    )

  #Calcul montant forfait logement
  fl_RSA <<- (((nb_adultes + nb_enfants) == 1) *
    bareme_var[["forf_logement_rsa_1"]]) +
    (((nb_adultes + nb_enfants) == 2) * bareme_var[["forf_logement_rsa_2"]]) +
  ...
  if (year > 15) {
    mf_PA <<- ifelse(
      ...
      ifelse(
        (nb_adultes == 1) && (maj_isole_rsa == 1),
        bareme_var[["montant_forfaitaire_PA_majo"]] *
          (1 +
            pmin(nb_enfants, 2) * bareme_var[["tx_majo_rsa_majo_enf12"]] +
            pmax(nb_enfants - 2, 0) * bareme_var[["tx_majo_rsa_majo_enf3"]]),
        0
    ...
    #Calcul montant forfait logement
    fl_PA <<- (((nb_adultes + nb_enfants) == 1) *
      bareme_var[["forf_logement_PA_1"]]) +
      (((nb_adultes + nb_enfants) == 2) * bareme_var[["forf_logement_PA_2"]]) +
  ...
  # Dont majo pour age
  af_majo_age <<- ifelse(
    (nb_enfants - nb_enft_20) >= 3,
    (nb_enft_1519 + nb_enft_14) * bareme_var[["majo_age_Af"]],
    0
  ) +
    ifelse(
      (nb_enfants - nb_enft_20 == 2) && (nb_enft_1519 + nb_enft_14 == 2),
  ...
  # Montant AF
  montant_af <<- ifelse(
    (nb_enfants - nb_enft_20) >= 2,
    bareme_var[["AF_2enft"]] +
      pmax(0, bareme_var[["AF_enft_sup"]] * (nb_enfants - nb_enft_20 - 2)) +
      ifelse(
        (nb_enft_20 >= 1) && (nb_enfants >= 3),
        bareme_var[["AF_forf_20ans"]],
        0
      ) +
      af_majo_age,
    0
  ...
  plaf_1tranch_AF <<- bareme_var[["p1_modulation_af"]] +
    ifelse(
      (nb_enfants - nb_enft_20) > 2,
      (nb_enfants - nb_enft_20 - 2) * bareme_var[["sup_enf_modulation_af"]],
      0
    )
  +ifelse(
    (nb_enft_20) >= 1 && (nb_enfants) > 2,
    nb_enft_20 * bareme_var[["sup_enf_modulation_af"]],
    0
  )

  # Plafond 2eme tranche AF
  plaf_2tranch_AF <<- bareme_var[["p2_modulation_af"]] +
    ifelse(
      (nb_enfants - nb_enft_20) > 2,
      (nb_enfants - nb_enft_20 - 2) * bareme_var[["sup_enf_modulation_af"]],
      0
  ...
  al_LC <<- ifelse(
    ...
    ifelse(
      nb_enfants == 1,
      bareme_var[["LC_1_pac"]],
      bareme_var[["LC_1_pac"]] + bareme_var[["Lc_supp_pac"]] * (nb_enfants - 1)
    )
  )
      ...
      ifelse(
        ...
        ifelse(
          ...
          ifelse(
            ...
            bareme_var[["TF_4pac"]],
            bareme_var[["TF_4pac"]] +
              bareme_var[["Tf_pers_sup"]] * (nb_enfants - 4)
          )
        )
      )
    )
          ...
          ifelse(
            ...
            ifelse(
              ...
              ifelse(
                ...
                bareme_var[["R0_6pac"]],
                bareme_var[["R0_6pac"]] +
                  bareme_var[["R0_pers_sup"]] * (nb_enfants - 6)
              )
            )
          )
        )
  ...
  if (year > 18) {
    cs_emp <<- function(x, bareme) {
      y <<- ifelse(
        x > 0,
        bareme[["taux_cs_emp_t1"]] * pmin(x, bareme[["plafond_ss"]]),
        0
      ) +
        ifelse(
          x > bareme[["plafond_ss"]],
          bareme[["taux_cs_emp_t2"]] *
            pmin(x - bareme[["plafond_ss"]], 2 * bareme[["plafond_ss"]]),
          0
        ) +
        ifelse(
          x > 3 * bareme[["plafond_ss"]],
          bareme[["taux_cs_emp_t3"]] *
            pmin(x - 3 * bareme[["plafond_ss"]], 1 * bareme[["plafond_ss"]]),
          0
        ) +
        ifelse(
          x > 4 * bareme[["plafond_ss"]],
          bareme[["taux_cs_emp_t4"]] *
            pmin(x - 4 * bareme[["plafond_ss"]], 4 * bareme[["plafond_ss"]]),
          0
        ) +
        ifelse(
          x > 8 * bareme[["plafond_ss"]],
          bareme[["taux_cs_emp_t5"]] * (x - 8 * bareme[["plafond_ss"]]),
          0
        ) +
        ifelse(
          x > bareme[["plafond_ss"]],
          bareme[["emp_retraites_comp_cet"]] *
            pmin(x, 8 * bareme[["plafond_ss"]]),
          0
        )
      return(y)
    }
    ...
    cs_emp <<- function(x, bareme) {
      y <<- ifelse(
        ...
        ifelse(
          x > 4 * bareme[["plafond_ss"]],
          bareme[["taux_cs_emp_t4"]] * (x - 4 * bareme[["plafond_ss"]]),
          0
        )
      return(y)
    }
  ...
  if (year > 14) {
    if (year > 18) {
      cotis_emp_conj <<- as.numeric(
        cs_emp(sal_brut_conjoint, bareme_var) -
          SI(
            part_smic_conj <= bareme_var[["part_smic_exo"]] * 100,
            bareme_var[["mod_cotis_fam"]] * sal_brut_conjoint,
            0
          ) -
          SI(
            part_smic_conj <= bareme_var[["seuil_smic_pat_assmal"]] * 100,
            bareme_var[["emp_maladie_t1"]] * sal_brut_conjoint,
            0
          )
      ) # Cotisations sociales employeurs
    } else {
  ...
  if (year > 18) {
    cs_sal <<- function(x, bareme) {
      y <<- (x > 0) *
        bareme[["taux_cs_sal_t1"]] *
        pmin(x, bareme[["plafond_ss"]]) +
        (x > bareme[["plafond_ss"]]) *
          bareme[["taux_cs_sal_t2"]] *
          pmin(x - bareme[["plafond_ss"]], 2 * bareme[["plafond_ss"]]) +
        (x > 3 * bareme[["plafond_ss"]]) *
          bareme[["taux_cs_sal_t3"]] *
          pmin(x - 3 * bareme[["plafond_ss"]], 1 * bareme[["plafond_ss"]]) +
        (x > 4 * bareme[["plafond_ss"]]) *
          bareme[["taux_cs_sal_t4"]] *
          pmin(x - 4 * bareme[["plafond_ss"]], 4 * bareme[["plafond_ss"]]) +
        (x > 8 * bareme[["plafond_ss"]]) *
          bareme[["taux_cs_sal_t5"]] *
          (x - 8 * bareme[["plafond_ss"]]) +
        ifelse(
          x > bareme[["plafond_ss"]],
          bareme[["sal_retraites_comp_cet"]] *
            pmin(x, 8 * bareme[["plafond_ss"]]),
          0
        )
      return(y)
  ...
  sal_declar_conj <<- (nb_adultes == 2) *
    (sal_brut_conjoint - cotis_sal_conj - csg_deduc_conj)

  # Cout du travail du conjoint
  cout_trav_conj <<- (nb_adultes == 2) *
    (sal_brut_conjoint + cotis_emp_conj - exo_fillon_conj)

  # Salaire mensuel net du conjoint
  sal_net_conj <<- ifelse(
    nb_adultes == 2,
    sal_brut_conjoint - cotis_sal_conj - csg_deduc_conj - csg_non_deduc_conj,
    0
  )
  ...
  nb_part <<- nb_adultes +
    ...
    (nb_adultes == 1) * (nb_enfants > 0) * 0.5

  # Avantage maximal lie au quotien familial
  max_qf <<- bareme_var[["plafond_qf"]] *
    (nb_part - nb_adultes) *
    2 +
    ifelse(
      (nb_adultes == 1) && (nb_enfants > 0),
      bareme_var[["plaf_qf_monoparent"]] - 2 * bareme_var[["plafond_qf"]],
      0
    )
  ...
  # Montant
  if (year < 23) {
    montant_TH <<- (nb_adultes == 1) *
      bareme_var[["montant_th_1pers"]] +
      (nb_adultes != 1) * bareme_var[["mont_th_couple"]]
  } else {
    montant_TH <<- 0
  }
  ...
  # Abattement RFR
  if (year < 20) {
    abat_rfr <<- bareme_var[["abt_th_base"]] +
      bareme_var[["abt_th_4demipart"]] * pmin(2, nb_part - 1) * 2 +
      bareme_var[["abt_th_demipartsup"]] * pmax(0, nb_part - 3) * 2
  } else {
    abat_rfr <<- 0
  }
  ...
  if (year < 20) {
    plaf_elig_th <<- bareme_var[["plaf_th_base"]] +
      bareme_var[["plaf_th_1demipart"]] * pmin(1.5, nb_part - 1) * 2 +
      bareme_var[["plaf_th_demipartsup"]] * pmax(0, nb_part - 1.5) * 2
  } else {
    plaf_elig_th <<- 0
  }
  ...
  if (year > 17 & year < 23) {
    seuil_rfr_degr_tot <<- bareme_var[["seuil_degrev_th_max_1p"]] +
      pmin(nb_part - 1, 2) * 2 * bareme_var[["supp_th_max_2demipart"]] +
      pmax(nb_part - 2, 0) * 2 * bareme_var[["supp_th_max_demipartsup"]]
  } else {
  ...
  if (year > 17 & year < 23) {
    seuil_rfr_degr_deg <<- bareme_var[["seuil_degrev_th_deg_1p"]] +
      pmin(nb_part - 1, 2) * 2 * bareme_var[["supp_th_deg_2demipart"]] +
      pmax(nb_part - 2, 0) * 2 * bareme_var[["supp_th_deg_demipartsup"]]
  } else {
  ...
  if (year < 23) {
    plaf_cmuc <<- bareme_var[["plaf_cmuc_base"]] +
      ...
      pmax(0, pmin(1, nb_adultes + nb_enfants - 2)) *
        bareme_var[["plaf_cmuc_34pac"]] +
      pmax(0, pmin(1, nb_adultes + nb_enfants - 3)) *
        bareme_var[["plaf_cmuc_34pac"]] +
      pmax(0, (nb_adultes + nb_enfants - 4)) *
        bareme_var[["plaf_cmuc_5pluspac"]]
  } else {
    plaf_cmuc <<- 0
  }
  ...
  if (year > 13) {
    # Montant total
    montant_asf <<- (eligible_asf == 1) *
      (nb_adultes == 1) *
      bareme_var[["mont_ASF_total"]] *
      nb_enfants
    # Entrant dans la base ressources du RSA
    asf_br_rsa <<- (eligible_asf == 1) *
      (nb_adultes == 1) *
      bareme_var[["mont_ASF_RSA"]] *
      nb_enfants
  } else {
    montant_asf <<- 0
    asf_br_rsa <<- 0
  ...
  if (year > 14) {
    ...
    sal_ref_conj_are <<- (nb_adultes == 2) *
      (are_nette_conjoint / bareme_var[["rap_salnet_are"]])
  } else {
    sal_ref_conj_are <<- 0
  }
  ...
  if (year > 23) {
    plaf_AAH <<- bareme_var[["AAH_plaf_seul"]] +
      bareme_var[["AAH_plaf_sup_pac"]] * nb_enfants
  } else {
    if (year > 14) {
      plaf_AAH <<- (nb_adultes == 1) *
        bareme_var[["AAH_plaf_seul"]] +
        (nb_adultes != 1) * bareme_var[["AAH_plaf_couple"]] +
        bareme_var[["AAH_plaf_sup_pac"]] * nb_enfants
    } else {
      plaf_AAH <<- 0
    }
  ...
  if (year > 16 & year < 20) {
    # 1er plafond
    prem_plaf_RI <<- (nb_adultes > 1) *
      (bareme_var[["plaf1_RI_couple"]] +
        bareme_var[["RI_demipart"]] * (nb_part - 2)) +
      (nb_adultes <= 1) *
        (bareme_var[["plaf1_RI"]] + bareme_var[["RI_demipart"]] * (nb_part - 1))
    #2e plafond
    deux_plaf_RI <<- (nb_adultes > 1) *
      (bareme_var[["plaf2_RI_couple"]] +
        bareme_var[["RI_demipart"]] * (nb_part - 2)) +
      (nb_adultes <= 1) *
        (bareme_var[["plaf2_RI"]] + bareme_var[["RI_demipart"]] * (nb_part - 1))
  } else {
    prem_plaf_RI <<- 0
    deux_plaf_RI <<- 0
  }
  ...
  if (year < 16) {
    ...
    plaf_rfr_ppe <<- ifelse(
      ...
      bareme_var[["plaf_ppe_suppl_enf"]] * pmin(2, nb_enfants) +
      2 * bareme_var[["plaf_ppe_suppl_enf"]] * pmax(0, nb_enfants - 2)
  ...
  uc_energie <<- (nb_adultes == 1) *
    (1 + (nb_enfants > 0) * (0.5 + (nb_enfants - 1) * 0.3)) +
    (nb_adultes > 1) * (1.5 + nb_enfants * 0.3)
