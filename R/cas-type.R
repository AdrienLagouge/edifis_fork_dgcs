...
castype <- function(vecteur, bareme_var, year) {
  ...
  if (year > 14) {
    if (year > 18) {
      cotis_emp <- cs_emp(vecteur, bareme_var) -
        SI(
          percen_smic_tps_plein <= bareme_var[["part_smic_exo"]] * 100,
          bareme_var[["mod_cotis_fam"]] * vecteur,
          0
        ) -
        SI(
          percen_smic_tps_plein <= bareme_var[["seuil_smic_pat_assmal"]] * 100,
          bareme_var[["emp_maladie_t1"]] * vecteur,
          0
  ...
  fillon_exo <- exo_fillon(
    vecteur,
    tps_travail,
    percen_smic_tps_plein,
    bareme_var
  ) # Exoneration Fillon
  fillon_exo[1] <- 0
  ...
  if (year < 16) {
    ...
    ppe_tps_trav <- ifelse(
      perc_smic_net_tpsplein < 50,
      "Tps partielc",
      ifelse(perc_smic_net_tpsplein < 100, "Tps partiell", "Tps plein")
    )
    ...
    ppe_declar_tps_plein <- SI(ppe_elig_rfr == 1, 1, 0) *
      SI(
        rev_act_dec / (1 + bareme_var[["deflat"]]) < bareme_var[["min_ppe"]],
        0,
        SI(
          (rev_act_dec / ((1 + bareme_var[["deflat"]]) * tps_trav_net / 100)) <=
            bareme_var[["sommet_ppe"]],
          bareme_var[["tx_ppe_avant"]] *
            (rev_act_dec / ((1 + bareme_var[["deflat"]]) * tps_trav_net / 100)),
          SI(
            (rev_act_dec /
              ((1 + bareme_var[["deflat"]]) * tps_trav_net / 100)) <=
              bareme_var[["max_ppe"]],
            (bareme_var[["max_ppe"]] -
              (rev_act_dec /
                ((1 + bareme_var[["deflat"]]) * tps_trav_net / 100))) *
              bareme_var[["tx_ppe_apres"]],
            0
          )
        )
      )
    ...
    if (all(rev_act_dec == rep(0, nn))) {
      ppe_declar_tps_plein <- 0
    } #nécessaire pour les mm raisons (correspond au cas où l'individu est à l'ARE)
    ...
    ppe_prime_indiv <- SI(ppe_elig_rfr == 1, 1, 0) *
      SI(
        ...
        SI(
          ...
          (1 - bareme_var[["majo_tp_ppe"]]) *
            perc_smic_net_tpsplein /
            100 +
            bareme_var[["majo_tp_ppe"]],
          1
        )
      ) *
    ...
    if (nb_adultes > 1 && sal_brut_conjoint > 0) {
      ppe_conj_tps_plein <- (ppe_elig_rfr == 1) *
        SI(
          sal_net_conj / (1 + bareme_var[["deflat"]]) < bareme_var[["min_ppe"]],
          0,
          SI(
            (sal_net_conj /
              ((1 + bareme_var[["deflat"]]) * tps_trav_conjoint)) <=
              bareme_var[["sommet_ppe"]],
            bareme_var[["tx_ppe_avant"]] *
              (sal_net_conj /
                ((1 + bareme_var[["deflat"]]) * tps_trav_conjoint)),
    ...
    ppe_majo_monoact <- SI(ppe_elig_rfr == 1, 1, 0) *
      SI(
        ...
        0,
        SI(
          ((sal_net_conj / (1 + bareme_var[["deflat"]]) <
            bareme_var[["min_ppe"]]) *
            (bareme_var[["min_ppe"]] <
              rev_act_dec / (1 + bareme_var[["deflat"]])) *
            (rev_act_dec / (1 + bareme_var[["deflat"]]) <
              bareme_var[["max_mono_ppe_inf"]]) |
            (rev_act_dec / (1 + bareme_var[["deflat"]]) <
              bareme_var[["min_ppe"]]) *
              (bareme_var[["min_ppe"]] <
                sal_net_conj / (1 + bareme_var[["deflat"]])) *
    ...
    ppe_majo_PAC <- SI(ppe_elig_rfr == 1, 1, 0) *
      (SI(
        ...
        SI(
          (ppe_prime_indiv > 0 | ppe_prime_conj > 0),
          nb_enfants * bareme_var[["majo_enf_ppe"]],
          SI(
            (bareme_var[["min_ppe"]] < sal_net_conj) *
              (sal_net_conj < bareme_var[["max_mono_ppe"]]) |
              (bareme_var[["min_ppe"]] <
                rev_act_dec / (1 + bareme_var[["deflat"]])) *
                (rev_act_dec / (1 + bareme_var[["deflat"]]) <
                  bareme_var[["max_mono_ppe"]]),
        ...
        SI(
          ...
          SI(
            ...
            bareme_var[["majo_monoparent_ppe"]] +
              (nb_enfants - 1) * bareme_var[["majo_enf_ppe"]],
            SI(
              (bareme_var[["min_ppe"]] <
                rev_act_dec / (1 + bareme_var[["deflat"]])) *
                (rev_act_dec / (1 + bareme_var[["deflat"]]) <
  ...
  # Montant ASS
  mont_ASS <- (nb_adultes == 2) *
    recours_conjoint_ASS *
    ((br_ASS < bareme_var[["ASS_plaf_couple"]]) *
      bareme_var[["ASS_mtt_forf"]] +
      (br_ASS >= bareme_var[["ASS_plaf_couple"]]) *
        (bareme_var[["ASS_diff_plaf_couple"]] - br_ASS >=
          bareme_var[["seuil_vers_ASS"]]) *
        (bareme_var[["ASS_diff_plaf_couple"]] - br_ASS))
  ...
  if (year < 22) {
    br_conj_AAH <- (rev_act_dec *
      bareme_var[["AAH_abatt_general"]] +
      autres_rev +
      are_nette_conjoint +
      ARE_net) *
      bareme_var[["abatt_rfr"]] +
      pmin(sal_declar_conj, bareme_var[["AAH_plafond_abatt_salaire"]]) *
        bareme_var[["AAH_abatt_salaire_t1"]] +
      pmax(sal_declar_conj - bareme_var[["AAH_plafond_abatt_salaire"]], 0) *
        bareme_var[["AAH_abatt_salaire_t2"]]

    #Montant (conjoint)
    mont_conj_AAH <- (handicap_conjoint > 0) *
      SI(
        plaf_AAH - br_conj_AAH >= bareme_var[["AAH_montant"]],
        bareme_var[["AAH_montant"]],
        pmax(0, plaf_AAH - br_conj_AAH)
      )
    ...
    if (year > 23) {
      ...
      br_conj_AAH <- pmax(
        ...
        rev_act_dec *
          bareme_var[["abatt_rfr"]] -
          bareme_var[["AAH_abatt_general"]] -
          nb_enfants * bareme_var[["AAH_abatt_majo"]]
      )
      +(autres_rev + are_nette_conjoint + ARE_net) *
        bareme_var[["abatt_rfr"]] +
        pmin(sal_declar_conj, bareme_var[["AAH_plafond_abatt_salaire"]]) *
          bareme_var[["AAH_abatt_salaire_t1"]] +
        pmax(sal_declar_conj - bareme_var[["AAH_plafond_abatt_salaire"]], 0) *
  ...
  ASS_conj <- (mont_ASS > 0) *
    SI(year > 16, (mont_ASS == max_aah_ass_conj), 1) *
    bareme_var[["noel_ASS"]]
  ...
  if (year > 14) {
    ...
    rev_impo_n_1 <- (rev_act_dec +
      ...
      SI(
        year < 17,
        mont_ASS,
        SI(mont_conj_AAH == max_aah_ass_conj, 0, mont_ASS)
      )) /
      (1 + bareme_var[["deflat"]]) *
      bareme_var[["abatt_rfr"]]

    # Revenu imposable (N-2)
    rev_impo_n_2 <- (rev_act_dec +
      autres_rev +
      sal_declar_conj +
      sal_ref_net +
      sal_ref_conj_are +
    ...
    revimp_n_2_abatt <- (rev_act_dec +
      ...
      (1 - bareme_var[["abatt_rbg_are"]]) * (sal_ref_net + sal_ref_conj_are)) /
      (1 + bareme_var[["deflat_2"]]) *
      bareme_var[["abatt_rfr"]]
  } else {
    rev_impo_n_1 <- (rev_act_dec + autres_rev + sal_declar_conj) *
      0.9 /
      (1 + bareme_var[["deflat"]])

    rev_impo_n_2 <- (rev_act_dec + autres_rev + sal_declar_conj) *
      0.9 /
      (1 + bareme_var[["deflat_2"]])
    ...
    BR_AL <- ceiling_dec(
      12 *
        ((1 + bareme_var[["deflat_2"]]) *
          ...
          mont_ASS -
          (nb_adultes == 2) *
            (rev_act_dec > bareme_var[["bmaf"]]) *
            (sal_declar_conj > bareme_var[["bmaf"]]) *
            bareme_var[["APL_abatt_biactifs"]]),
      -2
    ) /
      12
      ...
      (proprietaire == 0) *
        (pmax(
          ...
          al_LC -
            (al_PO +
              12 *
                (al_TF + bareme_var[["al_taux_compl"]]) *
                pmax(0, BR_AL - al_RO)) -
            bareme_var[["Mfo_deduire"]]
        ) *
          (1 - bareme_var[["tx_crds"]]) >=
          bareme_var[["seuil_versement_AL"]]) *
        pmax(
          0,
          al_LC -
            (al_PO +
      ...
      BR_AL <- ceiling_dec(
        12 *
          (revimp_n_2_abatt -
            mont_ASS -
            (nb_adultes == 2) *
              (rev_act_dec / (1 + bareme_var[["deflat_2"]]) >
                bareme_var[["bmaf_n_2"]]) *
              (sal_declar_conj / (1 + bareme_var[["deflat_2"]]) >
                bareme_var[["bmaf_n_2"]]) *
              bareme_var[["APL_abatt_biactifs"]]),
        -2
      ) /
            ...
            al_LC -
              (al_PO +
                12 *
                  ...
                  pmax(
                    0,
                    (rev_act_dec + sal_declar_conj + autres_rev) *
                      0.9 /
                      (1 + bareme_var[["deflat_2"]]) -
                      al_RO
                  ))
  ...
  if (year > 13) {
    ...
    plaf_CF <- SI(
      (nb_adultes == 1) |
        (rev_act_dec / (1 + bareme_var[["deflat_2"]]) >
          ...
          (sal_declar_conj / (1 + bareme_var[["deflat_2"]]) >
            bareme_var[["bmaf_n_2"]]),
      bareme_var[["plafond_maj_CF"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["pers_sup_CF"]],
      bareme_var[["plafond_simple_CF"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["pers_sup_CF"]]
    )
    ...
    plaf_CF_majo <- SI(
      (nb_adultes == 1) |
        (rev_act_dec / (1 + bareme_var[["deflat_2"]]) >
          ...
          (sal_declar_conj / (1 + bareme_var[["deflat_2"]]) >
            bareme_var[["bmaf_n_2"]]),
      bareme_var[["plafond_maj_CF_majo"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["pers_sup_CF_majo"]],
      bareme_var[["plafond_simple_CF_majo"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["pers_sup_CF_majo"]]
    )
    ...
    mont_CF <- SI(
      ...
      0,
      SI(
        revimp_n_2_abatt < plaf_CF_majo,
        bareme_var[["montant_CF_majo"]],
        SI(
          revimp_n_2_abatt < plaf_CF,
          bareme_var[["montant_CF"]],
          SI(
            revimp_n_2_abatt < (plaf_CF + bareme_var[["montant_CF"]]),
    ...
    mont_CF <- SI(
      ...
      0,
      SI(
        (rev_act_dec * 0.9 / (1 + bareme_var[["deflat_2"]])) < plaf_CF,
        bareme_var[["montant_CF"]],
        SI(
          (rev_act_dec * 0.9 / (1 + bareme_var[["deflat_2"]])) <
            (plaf_CF + bareme_var[["montant_CF"]]),
          bareme_var[["montant_CF"]] +
            plaf_CF -
            rev_act_dec * 0.9 / (1 + bareme_var[["deflat_2"]]),
          0
  ...
  if (year > 14) {
    ...
    plaf_AB_partiel <- SI(
      (nb_adultes == 1) |
        (rev_act_dec / (1 + bareme_var[["deflat_2"]]) >
          ...
          (sal_declar_conj / (1 + bareme_var[["deflat_2"]]) >
            bareme_var[["bmaf_n_2"]]),
      bareme_var[["plafond_majore_paje"]] +
        pmin(nb_enfants, 2) * bareme_var[["plafond_sup_enf_12_paje"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["plafond_sup_enf_3_paje"]],
      bareme_var[["plaf_simple_paje"]] +
        pmin(nb_enfants, 2) * bareme_var[["plafond_sup_enf_12_paje"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["plafond_sup_enf_3_paje"]]
    )
    ...
    plaf_AB_plein <- SI(
      (nb_adultes == 1) |
        (rev_act_dec / (1 + bareme_var[["deflat_2"]]) >
          ...
          (sal_declar_conj / (1 + bareme_var[["deflat_2"]]) >
            bareme_var[["bmaf_n_2"]]),
      bareme_var[["plafond_majore_paje_plein"]] +
        pmin(nb_enfants, 2) * bareme_var[["plafond_sup_enf_12_paje_plein"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["plafond_sup_enf_3_paje_plein"]],
      bareme_var[["plaf_simple_paje_plein"]] +
        pmin(nb_enfants, 2) * bareme_var[["plafond_sup_enf_12_paje_plein"]] +
        pmax(nb_enfants - 3, 0) * bareme_var[["plafond_sup_enf_3_paje_plein"]]
    )
    ...
    mont_AB_paje <- SI(
      revimp_n_2_abatt < plaf_AB_plein,
      bareme_var[["montant_paje_plein"]] * (nb_enft_3 > 0),
      SI(
        revimp_n_2_abatt < plaf_AB_partiel,
        bareme_var[["montant_paje_partiel"]] * (nb_enft_3 > 0),
        0
      )
    )
  } else {
    if (year == 14) {
      ...
      plaf_AB_plein <- SI(
        ...
        bareme_var[["plafond_majore_paje_plein"]] +
          pmin(nb_enfants, 2) * bareme_var[["plafond_sup_enf_12_paje_plein"]] +
          pmax(nb_enfants - 3, 0) *
            bareme_var[["plafond_sup_enf_3_paje_plein"]],
        bareme_var[["plaf_simple_paje_plein"]] +
          pmin(nb_enfants, 2) * bareme_var[["plafond_sup_enf_12_paje_plein"]] +
          pmax(nb_enfants - 3, 0) * bareme_var[["plafond_sup_enf_3_paje_plein"]]
      )
      ...
      mont_AB_paje <- SI(
        (rev_act_dec * 0.9 / (1 + bareme_var[["deflat_2"]])) < plaf_AB_partiel,
        bareme_var[["montant_paje_plein"]] * (nb_enft_3 > 0),
        0
      )
    }
  ...
  if (year < 22) {
    # Base ressources
    br_AAH <- ((sal_declar_conj + are_nette_conjoint) *
      bareme_var[["AAH_abatt_general"]] +
      autres_rev +
      ARE_net +
      SI(mont_conj_AAH == max_aah_ass_conj, 0, mont_ASS)) *
      bareme_var[["abatt_rfr"]] +
      pmin(rev_act_dec, bareme_var[["AAH_plafond_abatt_salaire"]]) *
        bareme_var[["AAH_abatt_salaire_t1"]] +
      pmax(rev_act_dec - bareme_var[["AAH_plafond_abatt_salaire"]], 0) *
        bareme_var[["AAH_abatt_salaire_t2"]]

    # Montant
    mont_AAH <- (handicap_personne > 0) *
      SI(
        plaf_AAH - br_AAH >= bareme_var[["AAH_montant"]],
        bareme_var[["AAH_montant"]],
        pmax(0, plaf_AAH - br_AAH)
      )

    # Majoration pour vie autonome
    mva <- (vecteur == 0) *
      (handicap_personne == 2) *
      (mont_AAH == bareme_var[["AAH_montant"]]) *
      (AL > 0) *
      bareme_var[["AAH_mva"]]

    #Majoration vie autonome (conjoint)
    mva_conj <- (sal_brut_conjoint == 0) *
      (handicap_conjoint == 2) *
      (mont_conj_AAH == bareme_var[["AAH_montant"]]) *
      (AL > 0) *
      bareme_var[["AAH_mva"]]
    ...
    if (year > 23) {
      ...
      br_AAH <- pmax(
        0,
        (sal_declar_conj + are_nette_conjoint) *
          bareme_var[["abatt_rfr"]] -
          bareme_var[["AAH_abatt_general"]] -
          nb_enfants * bareme_var[["AAH_abatt_majo"]]
      ) +
  ...
  if (year > 15) {
    ...
    br_rsa <- rev_act_net +
      ...
      AAH_tot +
      (AL > 0) * pmin(AL, fl_RSA) +
      (proprietaire == 1) * fl_RSA +
      montant_af -
      af_majo_age +
      pmin(mont_CF, bareme_var[["montant_CF"]]) +
      mont_AB_paje +
      asf_br_rsa +
      ARE_net +
      are_nette_conjoint +
      autres_rev +
      (mont_conj_AAH != max_aah_ass_conj) * mont_ASS

    # RSA socle
    mont_RSA <- (br_rsa + bareme_var[["seuil_versement_rsa"]] <= mf_RSA) *
      (mf_RSA - br_rsa)

    # Prime de Noel
    prime_noel <- (mont_RSA > 0) *
      (((nb_adultes + nb_enfants) == 1) *
        bareme_var[["noel_1pers"]] +
        ((nb_adultes + nb_enfants) == 2) * bareme_var[["noel_2pers"]] +
        ((nb_adultes + nb_enfants) == 3) * bareme_var[["noel_3pers"]] +
        (nb_adultes == 1) *
          ((nb_adultes + nb_enfants) == 4) *
          bareme_var[["noel_isole_4pers"]] +
        (nb_adultes == 2) *
          ((nb_adultes + nb_enfants) == 4) *
          bareme_var[["noel_couple_4pers"]] +
        (nb_adultes == 1) *
          ((nb_adultes + nb_enfants) > 4) *
          (bareme_var[["noel_isole_4pers"]] +
            bareme_var[["noel_pers_sup"]] * (nb_adultes + nb_enfants - 4)) +
        (nb_adultes == 2) *
          ((nb_adultes + nb_enfants) > 4) *
          (bareme_var[["noel_couple_4pers"]] +
            bareme_var[["noel_pers_sup"]] * (nb_adultes + nb_enfants - 4)))
    ...
    # Base ressources
    br_rsa <- pmin(mont_CF, bareme_var[["montant_CF"]]) +
      montant_af -
      af_majo_age +
      rev_act_net +
      pmin(AL, fl_RSA) * (AL > 0) +
      ...
      are_nette_conjoint +
      AAH_tot +
      (year == 15) * mont_AB_paje +
      sal_ref_net

    # RSA total
    RSA_tot <- SI(
      (recours_PA == 1 | br_rsa < mf_RSA),
      SI(
        mf_RSA -
          br_rsa +
          (1 - bareme_var[["pente_rsa"]]) * (rev_act_net + sal_net_conj) >=
          bareme_var[["seuil_versement_rsa"]],
        mf_RSA -
          br_rsa +
          (1 - bareme_var[["pente_rsa"]]) * (rev_act_net + sal_net_conj),
        0
      ...
      SI(
        mf_RSA - br_rsa >= bareme_var[["seuil_versement_rsa"]],
        mf_RSA - br_rsa,
        0
      )
    )
  ...
  if (year > 15) {
    ...
    br_pa <- rev_act_net +
      ...
      SI(
        year == 16,
        AAH_tot + mont_ASS,
        AAH_tot - mont_conj_AAH + max_aah_ass_conj
      ) +
      (AL > 0) * pmin(AL, fl_PA) +
      (proprietaire == 1) * fl_PA +
      montant_af -
      af_majo_age +
      pmin(mont_CF, bareme_var[["montant_CF"]]) +
      mont_AB_paje +
    ...
    bonus_pa <- bareme_var[["PA_bonus"]] *
      (SI(
        (rev_act_net +
          (rev_act_net >= bareme_var[["Sal_min_AAH_PA"]]) * mont_AAH) >=
          bareme_var[["PA_tranche1smic"]] * bareme_var[["smic_n"]],
        (pmin(
          rev_act_net +
            (rev_act_net >= bareme_var[["Sal_min_AAH_PA"]]) * mont_AAH,
          bareme_var[["PA_tranche2smic"]] * bareme_var[["smic_n"]]
        ) -
          bareme_var[["PA_tranche1smic"]] * bareme_var[["smic_n"]]) /
          ((bareme_var[["PA_tranche2smic"]] - bareme_var[["PA_tranche1smic"]]) *
            bareme_var[["smic_n"]]),
        0
      ) +
        SI(
          sal_net_conj +
            (sal_net_conj >= bareme_var[["Sal_min_AAH_PA"]]) *
              (mont_conj_AAH > mont_ASS) *
              mont_conj_AAH >=
            bareme_var[["PA_tranche1smic"]] * bareme_var[["smic_n"]],
          (pmin(
            sal_net_conj +
            ...
            bareme_var[["PA_tranche1smic"]] * bareme_var[["smic_n"]]) /
            ((bareme_var[["PA_tranche2smic"]] -
              bareme_var[["PA_tranche1smic"]]) *
              bareme_var[["smic_n"]]),
          0
        ))

    # Prime d'activite
    mont_PA <- (recours_PA == 1 | mont_RSA > 0) *
      SI(
        (vecteur + sal_net_conj > 0) *
          ((mf_PA +
            (1 - bareme_var[["PA_pente"]]) *
              (rev_act_net +
                sal_net_conj +
                (rev_act_net >= bareme_var[["Sal_min_AAH_PA"]]) * mont_AAH +
                (sal_net_conj >= bareme_var[["Sal_min_AAH_PA"]]) *
  ...
  if (year > 14) {
    ars <- SI(
      revimp_n_2_abatt <
        bareme_var[["plaf_ars"]] + bareme_var[["plaf_ars_enf"]] * nb_enfants,
      (bareme_var[["ars6_10"]] *
        nb_enft_610 +
        bareme_var[["ars_11_14"]] * (nb_enft_1113 + nb_enft_14) +
        bareme_var[["ars_15_18"]] * nb_enft_1519) /
        12,
      SI(
        revimp_n_2_abatt <
          bareme_var[["plaf_ars"]] +
            bareme_var[["plaf_ars_enf"]] * nb_enfants +
            (bareme_var[["ars6_10"]] *
              nb_enft_610 +
              bareme_var[["ars_11_14"]] * (nb_enft_1113 + nb_enft_14) +
              bareme_var[["ars_15_18"]] * nb_enft_1519) /
    ...
    ars <- SI(
      (revimp_n_2_abatt + rev_act_dec + sal_declar_conj + autres_rev) *
        0.9 /
        (1 + bareme_var[["deflat_2"]]) <
        bareme_var[["plaf_ars"]] + bareme_var[["plaf_ars_enf"]] * nb_enfants,
      (bareme_var[["ars6_10"]] *
  ...
  IR <<- function(rfr, nb_part, nb_adultes) {
    ...
    if (year > 14) {
      imp_par_part <- (rfr_par_part > 0) *
        bareme_var[["taux_ir_t1"]] *
        pmin(rfr_par_part, bareme_var[["plafond_ir_t1"]]) +
        (bareme_var[["plafond_ir_t1"]] < rfr_par_part) *
          bareme_var[["taux_ir_t2"]] *
          pmin(
            rfr_par_part - bareme_var[["plafond_ir_t1"]],
            bareme_var[["plafond_ir_t2"]] - bareme_var[["plafond_ir_t1"]]
          ) +
        (bareme_var[["plafond_ir_t2"]] < rfr_par_part) *
          bareme_var[["taux_ir_t3"]] *
          pmin(
            rfr_par_part - bareme_var[["plafond_ir_t2"]],
            bareme_var[["plafond_ir_t3"]] - bareme_var[["plafond_ir_t2"]]
          ) +
        (bareme_var[["plafond_ir_t3"]] < rfr_par_part) *
          bareme_var[["taux_ir_t4"]] *
          pmin(
            rfr_par_part - bareme_var[["plafond_ir_t3"]],
            bareme_var[["plafond_ir_t4"]] - bareme_var[["plafond_ir_t3"]]
          ) +
        (bareme_var[["plafond_ir_t4"]] < rfr_par_part) *
          bareme_var[["taux_ir_t5"]] *
          (rfr_par_part - bareme_var[["plafond_ir_t4"]])
    } else {
      imp_par_part <- (rfr_par_part > 0) *
        bareme_var[["taux_ir_t1"]] *
        pmin(rfr_par_part, bareme_var[["plafond_ir_t1"]]) +
        ...
        (bareme_var[["plafond_ir_t4"]] < rfr_par_part) *
          ...
          pmin(
            rfr_par_part - bareme_var[["plafond_ir_t4"]],
            bareme_var[["plafond_ir_t5"]] - bareme_var[["plafond_ir_t4"]]
          ) +
        (bareme_var[["plafond_ir_t5"]] < rfr_par_part) *
          bareme_var[["taux_ir_t6"]] *
          (rfr_par_part - bareme_var[["plafond_ir_t5"]])
    }
    ...
    if (year > 14) {
      imp_part_sansdemi <- (rev_imp_part > 0) *
        bareme_var[["taux_ir_t1"]] *
        pmin(rev_imp_part, bareme_var[["plafond_ir_t1"]]) +
        (bareme_var[["plafond_ir_t1"]] < rev_imp_part) *
          bareme_var[["taux_ir_t2"]] *
          pmin(
            rev_imp_part - bareme_var[["plafond_ir_t1"]],
            bareme_var[["plafond_ir_t2"]] - bareme_var[["plafond_ir_t1"]]
          ) +
        (bareme_var[["plafond_ir_t2"]] < rev_imp_part) *
          bareme_var[["taux_ir_t3"]] *
          pmin(
            rev_imp_part - bareme_var[["plafond_ir_t2"]],
            bareme_var[["plafond_ir_t3"]] - bareme_var[["plafond_ir_t2"]]
          ) +
        (bareme_var[["plafond_ir_t3"]] < rev_imp_part) *
          bareme_var[["taux_ir_t4"]] *
          pmin(
            rev_imp_part - bareme_var[["plafond_ir_t3"]],
            bareme_var[["plafond_ir_t4"]] - bareme_var[["plafond_ir_t3"]]
          ) +
        (bareme_var[["plafond_ir_t4"]] < rev_imp_part) *
          bareme_var[["taux_ir_t5"]] *
          (rev_imp_part - bareme_var[["plafond_ir_t4"]])
    } else {
      imp_part_sansdemi <- (rev_imp_part > 0) *
        bareme_var[["taux_ir_t1"]] *
        pmin(rev_imp_part, bareme_var[["plafond_ir_t1"]]) +
        ...
        (bareme_var[["plafond_ir_t5"]] < rev_imp_part) *
          bareme_var[["taux_ir_t6"]] *
          (rev_imp_part - bareme_var[["plafond_ir_t5"]])
    }
    ...
    if (year > 15) {
      decote <- pmax(
        ...
        SI(
          nb_adultes == 1,
          pmax(
            bareme_var[["Decote_taux"]] *
              bareme_var[["mont_decote_celib"]] -
              bareme_var[["Decote_taux"]] * imp_plaf_qf
          ),
          pmax(
            bareme_var[["Decote_taux"]] *
              bareme_var[["mont_decote_couple"]] -
              bareme_var[["Decote_taux"]] * imp_plaf_qf
          )
        )
      )
      ...
      if (year == 15) {
        ...
        decote <- SI(
          imp_plaf_qf < bareme_var[["plafond_decote"]],
      ...
      RI_2017 <- pmax(
        ...
        SI(
          ...
          SI(
            ...
            SI(
              (rfr > prem_plaf_RI) * (rfr <= deux_plaf_RI),
              (bareme_var[["tx_RI"]] * (deux_plaf_RI - rfr) * 12 / 4000) *
                (imp_plaf_qf - decote),
              0
            ),
          0
        ...
        pmax(
          ...
          SI(
            ...
            SI(
              ...
              SI(
                (rfr > prem_plaf_RI) * (rfr <= deux_plaf_RI),
                (bareme_var[["tx_RI"]] * (deux_plaf_RI - rfr) * 12 / 2000) *
                  (imp_plaf_qf - decote),
                0
              ),
            0
    ...
    if (year > 13) {
      imp_recouvr <- SI(
        imp_decote_RI > bareme_var[["seuil_recouvrement_IR"]],
        imp_decote_RI,
        0
      )
    } else {
      ...
      if (year > 13) {
        imp_ppe_recouvr <- SI(
          (imp_recouvr - ppe_resid >=
            bareme_var[["seuil_recouvrement_IR_apres_credits"]]) |
            (imp_recouvr - ppe_resid < 0),
          imp_recouvr - ppe_resid,
          0
        )
      } else {
  ...
  if (year > 18) {
    ...
    rfr2 <- (sal_declar_conj +
      sal_ref_conj_are +
      (mont_conj_AAH == max_aah_ass_conj) * 0 +
      (mont_conj_AAH != max_aah_ass_conj) * mont_ASS) *
      bareme_var[["abatt_rfr"]]
    rfr <- rfr1 + rfr2
  } else {
    rfr1 <- ((rev_act_dec + autres_rev + sal_ref_net) /
      (1 + bareme_var[["deflat"]])) *
      bareme_var[["abatt_rfr"]]
    rfr2 <- ((sal_declar_conj +
      sal_ref_conj_are +
  ...
  if (n0 == 3) {
    ...
    rfr_par_part <- sorties_IR1[(nn + 1):(2 * nn)] +
      sorties_IR2[(nn + 1):(2 * nn)]
    imp_par_part <- sorties_IR1[(2 * nn + 1):(3 * nn)] +
      sorties_IR2[(2 * nn + 1):(3 * nn)]
    imp_tot <- sorties_IR1[(3 * nn + 1):(4 * nn)] +
      sorties_IR2[(3 * nn + 1):(4 * nn)]
    rev_imp_part <- sorties_IR1[(4 * nn + 1):(5 * nn)] +
      sorties_IR2[(4 * nn + 1):(5 * nn)]
    imp_part_sansdemi <- sorties_IR1[(5 * nn + 1):(6 * nn)] +
      sorties_IR2[(5 * nn + 1):(6 * nn)]
    imp_tot_sansdemi <- sorties_IR1[(6 * nn + 1):(7 * nn)] +
      sorties_IR2[(6 * nn + 1):(7 * nn)]
    avantage_qf <- sorties_IR1[(7 * nn + 1):(8 * nn)] +
      sorties_IR2[(7 * nn + 1):(8 * nn)]
    imp_plaf_qf <- sorties_IR1[(8 * nn + 1):(9 * nn)] +
      sorties_IR2[(8 * nn + 1):(9 * nn)]
    decote <- sorties_IR1[(9 * nn + 1):(10 * nn)] +
      sorties_IR2[(9 * nn + 1):(10 * nn)]
    RI_2017 <- sorties_IR1[(10 * nn + 1):(11 * nn)] +
      sorties_IR2[(10 * nn + 1):(11 * nn)]
    imp_decote_RI <- sorties_IR1[(11 * nn + 1):(12 * nn)] +
      sorties_IR2[(11 * nn + 1):(12 * nn)]
    imp_recouvr <- sorties_IR1[(12 * nn + 1):(13 * nn)] +
      sorties_IR2[(12 * nn + 1):(13 * nn)]
    imp_ppe_recouvr <- sorties_IR1[(13 * nn + 1):(14 * nn)] +
      sorties_IR2[(13 * nn + 1):(14 * nn)]

    av_QC <- sorties_IR1[(12 * nn + 1):(13 * nn)] +
      sorties_IR2[(12 * nn + 1):(13 * nn)] -
      sorties_IR[(12 * nn + 1):(13 * nn)]
  } else {
    rfr <- sorties_IR[1:nn]
  ...
  if (year > 14) {
    ...
    if (year > 19) {
      ...
      plaf_mont_th <- pmax(0, revimp_n_2_abatt - abat_rfr) *
        bareme_var[["tx_degrevement_th"]]
      mont_TH_predegr <- (elig_plaf_th *
        pmin(plaf_mont_th, montant_TH) +
        (1 - elig_plaf_th) * montant_TH) *
        (1 -
          ((imp_recouvr == 0) *
            (handicap_personne > 0 | handicap_conjoint > 0)))
    }
    if (year > 17 & year < 23) {
      # Montant de TH
      mont_TH <- mont_TH_predegr *
        ((rev_impo_n_1 < seuil_rfr_degr_tot) *
          (1 - bareme_var[["degr_th_max"]]) +
          SI(
            (rev_impo_n_1 >= seuil_rfr_degr_tot) &
              (rev_impo_n_1 < seuil_rfr_degr_deg),
            (1 -
              bareme_var[["degr_th_max"]] *
                (seuil_rfr_degr_deg - rev_impo_n_1) /
                (seuil_rfr_degr_deg - seuil_rfr_degr_tot)),
            0
          ) +
          ...
          ((rev_impo_n_1 < seuil_rfr_degr_tot) *
            ...
            SI(
              ...
              (1 -
                (seuil_rfr_degr_deg - rev_impo_n_1) /
                  (seuil_rfr_degr_deg - seuil_rfr_degr_tot)) *
                (1 - bareme_var[["degr_th_partiel"]]),
              0
            ) +
            (rev_impo_n_1 >= pmax(seuil_rfr_degr_tot, seuil_rfr_degr_deg)) *
              (1 - bareme_var[["degr_th_partiel"]]))
      }
    } else {
      # Montant de TH
  ...
  if (year > 14) {
    mont_AF <- SI(
      revimp_n_2_abatt <= plaf_1tranch_AF,
      montant_af,
      SI(
        revimp_n_2_abatt <= plaf_1tranch_AF + montant_af / 2,
        montant_af + plaf_1tranch_AF - revimp_n_2_abatt,
        SI(
          revimp_n_2_abatt <= plaf_2tranch_AF,
          montant_af / 2,
          SI(
            revimp_n_2_abatt <= plaf_2tranch_AF + montant_af / 4,
  ...
  nv_vie <- rev_disp /
    (1 +
      (nb_adultes - 1 + nb_enft_14 + nb_enft_1519 + nb_enft_20) * 0.5 +
      (nb_enft_3 + nb_enft_35 + nb_enft_610 + nb_enft_1113) * 0.3)
  ...
  for (i in 1:(length(vecteur))) {
    TMI_net[i] <- (1 -
      ((rev_disp[i + 1] - rev_disp[i]) /
        (rev_act_net[i + 1] + ARE_net[i + 1] - rev_act_net[i] - ARE_net[i])))
  }
  ...
  for (i in 1:(length(vecteur))) {
    TMI_superbrut[i] <- 1 -
      ((rev_disp[i + 1] - rev_disp[i]) /
        (cout_travail[i + 1] - cout_travail[i]))
  }
  ...
  if (year > 13) {
    decile <- 1 +
      SI(bareme_var[["seuil_D1"]] < nv_vie * 12, 1, 0) +
      SI(bareme_var[["seuil_D2"]] < nv_vie * 12, 1, 0) +
      SI(bareme_var[["seuil_D3"]] < nv_vie * 12, 1, 0) +
      SI(bareme_var[["seuil_D4"]] < nv_vie * 12, 1, 0) +
      SI(bareme_var[["seuil_D5"]] < nv_vie * 12, 1, 0) +
      SI(bareme_var[["seuil_D6"]] < nv_vie * 12, 1, 0) +
      SI(bareme_var[["seuil_D7"]] < nv_vie * 12, 1, 0) +
      SI(bareme_var[["seuil_D8"]] < nv_vie * 12, 1, 0) +
  ...
  if (year > 15) {
    CMUc <- as.numeric(
      (recours_CMU_ACS *
        SI(
          ...
          (br_rsa < plaf_cmuc) *
            (nb_adultes *
              bareme_var[["montant_cmuc_16_49"]] +
              (nb_enfants - nb_enft_1519 - nb_enft_20) *
                bareme_var[["montant_cmuc_0_15"]] +
    ...
    if (year > 19) {
      CMUc <- as.numeric(
        (recours_CMU_ACS * SI(mont_RSA == 0, (br_rsa < plaf_cmuc), 1)) > 0
      )
    }
  } else {
    CMUc <- as.numeric(
  ...
  if (year > 19) {
    ACS <- as.numeric(
      (recours_CMU_ACS * (plaf_cmuc < br_rsa) * (br_rsa < plaf_acs)) > 0
    )
  } else {
    ACS <- as.numeric(
      (recours_CMU_ACS *
        ...
        (br_rsa < plaf_acs) *
        (nb_adultes *
          bareme_var[["montant_acs_16_49"]] +
          (nb_enfants - nb_enft_1519 - nb_enft_20) *
            bareme_var[["montant_acs_0_15"]] +
          (nb_enft_1519 + nb_enft_20) * bareme_var[["montant_acs_16_49"]])) >
        0
    )
  }
  ...
  if (year < 18) {
    energie <- recours_CMU_ACS *
      (CMUc > 0 | ACS > 0) *
      (((nb_adultes + nb_enfants) == 1) *
        bareme_var[["montant_elec_1p"]] +
        ((nb_adultes + nb_enfants) == 2 | (nb_adultes + nb_enfants) == 3) *
          bareme_var[["montant_elec_23p"]] +
        ((nb_adultes + nb_enfants) > 3) * bareme_var[["montant_elec_4plus"]])
  } else {
    if (year == 18) {
      energie <- ((rev_impo_n_1 / uc_energie <
        bareme_var[["plafond_ener_1tranch"]]) *
        ((uc_energie == 1) *
          (bareme_var[["ener_1tranch_1UC"]]) +
          SI(
            (uc_energie > 1) & (uc_energie < 2),
            bareme_var[["ener_1tranch_2UC"]],
            0
          ) +
          (uc_energie >= 2) * (bareme_var[["ener_1tranch_3UC"]])) +
        SI(
          (rev_impo_n_1 / uc_energie >= bareme_var[["plafond_ener_1tranch"]]) &
            (rev_impo_n_1 / uc_energie < bareme_var[["plafond_ener_2tranch"]]),
          (uc_energie == 1) *
            bareme_var[["ener_2tranch_1UC"]] +
            SI(
              (uc_energie > 1) & (uc_energie < 2),
              bareme_var[["ener_2tranch_2UC"]],
              0
            ) +
            (uc_energie >= 2) * bareme_var[["ener_2tranch_3UC"]],
          0
        ) +
        SI(
          (rev_impo_n_1 / uc_energie >= bareme_var[["plafond_ener_2tranch"]]) &
            (rev_impo_n_1 / uc_energie < bareme_var[["plafond_ener_3tranch"]]),
          (uc_energie == 1) *
            bareme_var[["ener_3tranch_1UC"]] +
            SI(
              (uc_energie > 1) & (uc_energie < 2),
              bareme_var[["ener_3tranch_2UC"]],
              0
            ) +
            (uc_energie >= 2) * bareme_var[["ener_3tranch_3UC"]],
          0
        )) /
        12
    } else {
      if (year == 22) {
        energie <- ((rev_impo_n_1 / uc_energie <
          ...
          ((uc_energie == 1) *
            ...
            (uc_energie >= 2) * (bareme_var[["ener_1tranch_3UC"]])) +
          SI(
            (rev_impo_n_1 / uc_energie >=
              bareme_var[["plafond_ener_1tranch"]]) &
              (rev_impo_n_1 / uc_energie <
                bareme_var[["plafond_ener_2tranch"]]),
            (uc_energie == 1) *
              bareme_var[["ener_2tranch_1UC"]] +
              SI(
                (uc_energie > 1) & (uc_energie < 2),
          ...
          SI(
            (rev_impo_n_1 / uc_energie >=
              bareme_var[["plafond_ener_2tranch"]]) &
              (rev_impo_n_1 / uc_energie <
                bareme_var[["plafond_ener_3tranch"]]),
            (uc_energie == 1) *
              bareme_var[["ener_3tranch_1UC"]] +
              SI(
                (uc_energie > 1) & (uc_energie < 2),
          ...
          SI(
            (rev_impo_n_1 / uc_energie >=
              bareme_var[["plafond_ener_3tranch"]]) &
              (rev_impo_n_1 / uc_energie <
                bareme_var[["plafond_ener_4tranch"]]),
            (uc_energie == 1) *
              bareme_var[["ener_4tranch_1UC"]] +
              SI(
                (uc_energie > 1) & (uc_energie < 2),
                bareme_var[["ener_4tranch_2UC"]],
                0
              ) +
              (uc_energie >= 2) * bareme_var[["ener_4tranch_3UC"]],
            0
          ) +
          SI(
            (rev_impo_n_1 / uc_energie >=
              bareme_var[["plafond_ener_4tranch"]]) &
              (rev_impo_n_1 / uc_energie <
                bareme_var[["plafond_ener_5tranch"]]),
            bareme_var[["ener_bonus2022"]],
            0
          )) /
          12
      } else {
  ...
  if (year > 15) {
    total_minima_soc_cheque_energie <- mont_ASS +
      AAH_tot +
      mont_RSA +
      prime_noel +
      mont_PA +
  ...
  if (year == 24) {
    df <- data.frame(
      ...
      mont_PA,
      total_minima_soc,
      total_minima_soc_cheque_energie,
      ars,
      total_pf,
      rfr,
      rfr_par_part,
...
castype1 <- function(data, leg, bareme_var, year, n2000) {
  ...
  if (n2000 == 0) {
    ...
    p1 <- fig %>%
      ...
      add_trace(
        x = c(data[floor((dim(data)[1] - 1) / 2), "rev_act_net"]),
        type = 'scatter',
        mode = 'lines',
        line = list(shape = 'linear', color = 'grey', dash = "dash"),
        name = 'Note de lecture',
        text = 'Note de lecture',
        hoverinfo = 'text'
      ) %>%
      layout(
        title = "Effet marginal sur le revenu disponible \n d'une augmentation de salaire net",
  ...
  if (n2000 == 1) {
    ...
    p1 <- fig %>%
      ...
      add_trace(
        x = c(data[floor((dim(data)[1] - 1) / 2), "ARE_net"]),
        type = 'scatter',
        mode = 'lines',
        line = list(shape = 'linear', color = 'grey', dash = "dash"),
        name = 'Note de lecture',
...
castype3 <- function(data, leg, bareme_var, year, mv, n2000) {
  ...
  if (n2000 == 0) {
    print(paste(
      ...
      system.time({
        tmp <- data %>%
          ...
          mutate(across(all_of(mv), as.numeric)) %>%
          mutate(
    ...
    p3 <- ggplot(data2, aes(x = rev_act_net, y = value)) +
      ...
      geom_line(
        data = data2,
        aes(rev_act_net, rev_disp, color = 'Revenu disponible'),
        linewidth = 1
      ) +
      geom_line(
        data = data.frame(
  ...
  if (n2000 == 1) {
    ...
    p3 <- ggplot(data2, aes(x = ARE_net, y = value)) +
      ...
      geom_line(
        data = data2,
        aes(ARE_net, rev_disp, color = 'Revenu disponible'),
        linewidth = 1
      ) +
      geom_line(
        data = data.frame(
